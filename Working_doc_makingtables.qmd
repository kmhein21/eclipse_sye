...
ADI_3<-ADI_2|>pivot_longer(cols = starts_with("ADI"), names_to = "WAV", values_to = "ADI")|>
...
```{r}
ggplot(data = ADI_3, aes(x=num, y = ADI))+
  geom_line(col = "green")+
  facet_wrap(~WAV)+
  theme_minimal()
...
ACI_3<-ACI_2|>pivot_longer(cols = starts_with("ACI"), names_to = "WAV", values_to = "ACI")|>
  mutate(WAV = as.factor(WAV), num = as.integer(num))|>
...
```{r}
ggplot(data=ACI_3, aes(x = num , y=ACI))+
