...
```{r}
Same_time_4_8<-Final_4_8|>
  separate(Paths_date, into = c("Date", "Time"), sep = "_")|>
  separate(Time, into = c("time", "temp"), sep = "\\.")|>
  separate(time, into = c("hours","other"), sep = 2)|>
  separate(other, into = c("min", "sec"), sep = 2)|>
  unite("time", c(,"hours", "min","sec"), sep = ":")|>
  mutate(time = hms(time))|>
  select(-temp)

Same_time_4_7<-Final_4_7|>
  separate(Paths_date, into = c("Date", "Time"), sep = "_")|>
  separate(Time, into = c("time", "temp"), sep = "\\.")|>
  separate(time, into = c("hours","other"), sep = 2)|>
  separate(other, into = c("min", "sec"), sep = 2)|>
...
ggplot(data = New_df_4_8, aes(x = time, y = Biophony))+
  geom_line()+
  geom_line(data = New_df_4_7, aes(x = time, y = Biophony))

ggplot(data = Same_time_4_8, aes( x = time, y = Biophony))+
  geom_line()+
  geom_line(data = Same_time_4_7, aes( x = time, y = Biophony), col = "orange")

```

Acoustic Evenness Index visual for the two days 
```{r}
ggplot(data = New_df_4_8, aes(x = time, y = AEI))+
  geom_line()+
  geom_line(data = New_df_4_7, aes( x = time, y = AEI))

ggplot(data = Same_time_4_8, aes( x = time, y = AEI))+
  geom_line()+
  geom_line(data = Same_time_4_7, aes( x = time, y = AEI), col = "orange")
```

Bioacoustic Index visual for the two days

```{r}
ggplot(data = New_df_4_8, aes(x = time, y = BEI))+
  geom_line()+
  geom_line(data = New_df_4_7, aes( x = time, y = BEI))

ggplot(data = Same_time_4_8, aes( x = time, y = BEI))+
  geom_line()+
  geom_line(data = Same_time_4_7, aes( x = time, y = BEI), col = "orange")
