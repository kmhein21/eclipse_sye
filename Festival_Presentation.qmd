...
single_day<-A001_SD001|>filter(day == "2024-04-08")
...
afternoon<- single_day|> filter(hour >= as_hms("19:00:00") & hour <= as_hms("21:00:00"))
night<- single_day|> filter(hour >= as_hms("23:00:00") & hour <= as_hms("23:30:00"))

ggplot(data = dawn, aes( x = hour, y = bei))+
  geom_rect( aes(xmin = as_hms("05:45:00"), xmax = as_hms("07:15:00"), ymin = 0, ymax = 10), fill = "lightblue1")+
  geom_rect(data = dawn, aes(xmin = as_hms("14:00:00"), xmax = as_hms("16:00:00"), ymin = 0, ymax = 10), fill = "lightblue1")+
  geom_rect(data = dawn, aes(xmin = as_hms("19:00:00"), xmax = as_hms("21:00:00"), ymin = 0, ymax = 10), fill = "lightblue1")+
  geom_rect(data = dawn, aes(xmin = as_hms("23:00:00"), xmax = as_hms("23:30:00"), ymin = 0, ymax = 10), fill = "lightblue1")+
...
* Time - Temporal autocorrelation: Adjusting for this through incorporation in the model and time series (Emilia Agostinelli)

* Species - What wildlife are being affected: Looking into the varying audio patterns and the possibility of utilizing machine learning (Aidan Fauth)
...
bold BEI and Biophony or callout boxes
change size of legend/text in model visualizations
reduce amount of text - specifically R and HPC 
change theme
Erika's students
