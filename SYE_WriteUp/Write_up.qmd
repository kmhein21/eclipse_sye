...
where y_i is the response, $\beta_0$ is an intercept, $f_j(x_{ij})$ is a smoothing function for predictor j and $\epsilon_i$ is assumed to be N(0, $\simga^2$) and independent of all other $\epsilon_i$
...
$$ \displaystyle{\Sigma_{i=1}^n}(y_i - g(x_i))^2 + \lambda \int g^{''}t^2  dt$$
