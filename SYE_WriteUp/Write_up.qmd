...
A high AEI value signifies a more even soundscape, within the audio there isn't any sound domination from one type of sound/species. A low AEI signifies that the sound is not as even, there may only be a few loud or similar sounds which disrupt the consistency of the sound level.

In the paper by Gerber et. al in 2017, this was found to be a statistically significant predictor in their model. They found that the AEI was highest right before the eclipse began and it rose until the period of totality. The authors believe this is likely due to the increase in human sounds near their recording locations (@gerber2020soundscape).
...
In our reference paper, this was found to be a non-significant predictor but, it seemed to be greatest at the period of totality (@gerber2020soundscape). 
...
Biophony is an index which calculates the average frequency of biotic sound (between 2-8kHz). The interpretation of this index is much more straightforward, high biophony means that there is more sound and low values mean less.

Gerber et al. found that biophony was a significant predictor in thier model They found that biophony increased at the beginning and end of the eclipse, similar to the dawn and dusk choruses (@gerber2020soundscape). 
...
Initially, we tested this function on two folders which are a part of the larger collection of data from audio recorder 4. The two folders we chose correspond to April 7th and 8th. Graphs from the indexes across the four different recording times are shown below
...
We started our analysis on the full time series, but this created a lot of crowding and was difficult to read. We decided to focus on the time interval 14:00:00 to 16:50:00, which corresponded to the eclipse. We then created two visuals: one which displays only the pattern from the day of the eclipse, and then a similar visual which shows all of the recording days. In these visuals we added a shaded region to show the time of totality (15:27:05 to 15:23:52). 
...
From the graph depicting biophony on the day of the eclipse, we can see there is a sharp decrease before totality begins, and during/after totality the biophony from this recorder seems to stay pretty low. When we look at the total recording days, there are days where the biophony is at similar levels. From this one folder, its hard to tell if biphony will be an index which shows potential patterns across the eclipse. 
...
On the day of the eclipse, we can see an increase of AEI right at the beginning of totality. This increase is one of the starkest increases in the full time interval. When we then look at all the days, we can see that this type of increase is pretty infrequent at this time. The pattern on the 8th during totality seems to stick out. 
...
For the ACI, there seems to be the potential for an interesting pattern,
...
This set of visuals allowed us to look into whether we could expect to observe any potential patterns across the time of the eclipse for our five indices. From these visuals, we felt confident that as we got into more visuals and modeling we would see some type of audiological changes during the time of the eclipse. 
...
For biophony, when we look at the smoothing curves across the days we can see that basically all of the days besides the 11th are quite linear. Since April 8th is so flat, this suggests that the eclipse may not be influencing this index. 
...
To try and get some more evidence, we looked at a model from the dawn. Like the other metrics we calculated dawn models for, April 8th displays a pretty linear trend line and a low edf value (1.514). Since the 8th doesn't vary too much from the other days, this shows April 8th was a regular day for acoustic diversity measurement. Like AEI though, this fact doesn't help us solidify any more evidence for the patterns we saw around totality. 
