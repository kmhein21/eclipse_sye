...
fullAudio<-rbind(A001_SD001, A002_SD013, A003_SD005, A004_SD012,A005_SD002, A006_SD006, A007_SD017, A008_SD007, A009_SD009, A010_SD014, A011_SD018, A013_SD016, A014_SD021, A015_SD010, A016_SD022)|>
...
From the visualization we decided to focus on specifically the Bioacoustic index as it seemed like this index which would show a clear connection to the eclipse. To maintain additivity and account for the non-linear patterns in the data we analyzed the data using Generalized Additive Models. These models were made using a smoothing spline for hour separated by day along with a function of day. 
...
From our model output we can see that using this spline on hour by day is significant in assessing the Bioacoustic index. From looking at the estimated degrees of freedom for the smooth terms we can see that April 8th posesses the most flexible trend line with an edf of 8.79. The other days vary in their flexibility but we can see that they all do have non-linear effects with April 11th being the most flexibile, but with about half the edf value as April 8th, at 4.033. 

We can also note from this output that the model has a explained deviance value of 11.1%, meaning that using our smoothing spline and day variable explain  11.1% of the variability in the bioacoustic index.
...
This plot is similar to the one above, but it compiles all of the daya on top of eachother for easier comparison. Here we can see how the flexibility in the Bioacoustic index on the 8th and how that compares to the rigidity of the other days. 
...
Using this model we can see that a majority of the splines are still significant, however the only non-significant spline is actually occuring on the 8th. In this summary we can also see based on the edf the 8th is actually the least flexible day during this time period. This helps us understand that there is not any special pattern that occured on the 8th and provides evidence that it was a normal day. 

We can also see that this model has an explained deviance value of 8.8%, meaning that our model explains 8.8% of the variability in the bioacoustic index during this time frame. 
