...
From our visualization, we decided to focus on specifically the Bioacoustic index as it seemed like this index which would show a clear connection to the eclipse. To maintain additivity and account for the non-linear patterns in the data we analyzed the data using Generalized Additive Models. These models were made using a smoothing spline for hour separated by day, an additional spline for folder_name which would help us account for the different audiomoths/locations, and lastly a function of day. 
...
When we plot the effects of each day we can see April 8th's flexibility. We can see a very high peak which does not appear in any of the other days, with the other days possessing some flexibility as we expected with no major maximums or minimums. 
...
This plot is similar to the one above, but it compiles all of the data on top of eachother for easier comparison. Here we can see how the flexibility in the Bioacoustic index on the 8th and how that compares to the rigidity of the other days. To produce this graph, we needed to specify one of our folders which lies close to the middle of the response ("A014_SD021") to create predictions using our model.  
