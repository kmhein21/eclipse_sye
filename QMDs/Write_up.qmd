...
TO DO:
 ...
 potentially replace the full data visualization with just raw eclipse data for better readability 
 
\newpage

## Abstract: 

From May 30 to April 16th, data from 20 audio recording devices was collected by Erika Barthelmess from the St. Lawrence Biology department. These devices were strategically located to observe potential audiological changes in wildlife across the time of the total solar eclipse on April 8th, 2024. Using R studio and the HPC, we processed this audio data to produce clean and organized data frames, centered around 5 commonly used bioacoustic indices. From this tabular data we were then able to construct many visualizations displaying patterns across, time, day, and recording device. Due to non-linearity associated with our plots, we then constructed Generalized Additive Models based on day and recording device to assess patterns that may have occurred, specifically during the time of the eclipse on April 8th. From these visuals, we conclude in this project the possibility that 3 of our 5 indices are associated with audio changes during the time of the eclipse, specifically around the time of totality. 



1. Data collection and methods - how many, how long they recorded from (Erika)
2. Using HPC and functions to create cleaned data frames to record 5 commonly used bioacoustic indices
3. Creation of visualizations, (For example bioacustic index)  
4. found patterns in 3 of the indices that may be associated with the eclipse, specifically around the time of totality
...
ggplot(data = dawn, aes( x = hour, y = bei))+
  ...
  theme_minimal()+
  facet_wrap(~day)+
  labs( title = "Full Bioacoustic Evenness Patterns Over all Days")+
...
Our final generalized additive models were made using a smoothing spline for hour separated by day, an additional smoothing spline for folder_name which would help us account for the different audiomoths/locations, and lastly a linear function of day. To create these models we used the gam() function from the mgcv packages, and to create our visualization we used the data_grid() function (from mgcv) as well as the augment() function from the broom package. This allowed us to create predicted values based on our model and to then visualize them alongside our GAMs.  
