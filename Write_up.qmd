...
When we display the data by day, we can see that most days have flat smoothing curves, with fluctuations only on the 8th and 6th. We can see that on the 8th, there is a much steeper rise around that time of totality, with minimum on either side of its beginning and end. Since we see some of that curvature on the 6th, it is hard for us to conclude from this visual whether the eclipse did influence Acoustic evenness or not. 
...
For biophony, when we look at the smoothing curves across the days we can see that the basically all of the days including April 8th are linear, this visual suggests that the eclipse may not be influencing this index. 
...
(CHANGE MODEL SUMMARIES) From our model output we can see from the F-values and p-values that our smoothing splines are extremely significant in the model. From looking at the estimated degrees of freedom for the smooth terms we can see that April 8th possesses an extremely complex line line with an edf of 8.861. The other days vary in their complexity but we can see that they all do have non-linear effects with April 7th being the second most flexible after April 8th, with an edf of 7.464. 
